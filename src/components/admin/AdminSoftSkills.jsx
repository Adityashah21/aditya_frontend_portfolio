...
export default function AdminSoftSkills() {
  ...
  const fetchAll  = () => api.get('/softskills').then(r => setItems(r.data)).catch(() => {})
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) { await api.put(`/softskills/${editing}`, form); showToast('Updated') }
      else         { await api.post('/softskills', form);           showToast('Added') }
  ...
  const handleDelete = async (id) => {
    ...
    try { await api.delete(`/softskills/${id}`); showToast('Deleted'); fetchAll() }
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        <div className="flex items-center justify-between mb-10 flex-wrap gap-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-orbitron font-bold text-3xl" style={{ color:'#ff00ff', textShadow:'0 0 10px #ff00ff66' }}>SOFT SKILLS</h1>
          </div>
          <button className="btn-cyber text-xs px-5 py-2" onClick={openAdd}>+ ADD SOFT SKILL</button>
        </div>
          ...
          {showForm && (
            <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
              ...
              <motion.div initial={{ scale:0.9, opacity:0 }} animate={{ scale:1, opacity:1 }} exit={{ scale:0.9, opacity:0 }}
                ...
                <h2 className="font-orbitron font-bold text-xl mb-6" style={{ color:'#ff00ff' }}>
                  {editing ? 'EDIT SOFT SKILL' : 'NEW SOFT SKILL'}
                </h2>
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                  <div><Label c="SKILL NAME *" /><Input value={form.name} onChange={set('name')} required placeholder="Communication" /></div>
                  <div>
                    <Label c="DESCRIPTION" />
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                      {ICONS.map(ic => (
                        <button key={ic} type="button" onClick={() => setForm(f => ({ ...f, icon: ic }))}
                          ...
                          style={{ borderColor: form.icon===ic ? '#ff00ff' : 'rgba(0,240,255,0.1)', background: form.icon===ic ? 'rgba(255,0,255,0.1)' : 'transparent' }}>
                          {ic}
                        </button>
                      ))}
                    </div>
        ...
        <div className="grid md:grid-cols-2 gap-3">
          {items.length === 0
            ? <p className="font-mono text-sm text-cyber-dim py-16 col-span-2 text-center">// No soft skills yet</p>
            : items.map((item, i) => (
